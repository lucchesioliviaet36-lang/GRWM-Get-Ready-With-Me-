...
import { ResponsesEmitter, WebSocketError, buildURL } from "./internal-base.mjs";
import { InternalEventEmitter } from "../../core/EventEmitter.mjs";
import { sleep } from "../../internal/utils/sleep.mjs";
import { ReadyState } from "../../internal/ws-adapter.mjs";
import { SendQueue, getMaxBufferedEvents, flattenRawData, isRecoverableClose, } from "../../internal/ws.mjs";
...
export class ResponsesWSBase extends ResponsesEmitter {
    ...
    [Symbol.asyncIterator](options) {
        ...
        const wakeResolver = () => {
            if (resolverHead >= resolvers.length)
                return;
        ...
        const push = (msg) => {
            ...
            if (maxBufferedEvents !== undefined && queue.length - queueHead >= maxBufferedEvents) {
                failure = new WebSocketError(`WebSocket stream exceeded maxBufferedEvents (${maxBufferedEvents})`, null);
                done = true;
                queue.length = 0;
                queueHead = 0;
                cleanup();
        ...
        const onSocketSwap = (_oldSocket, newSocket) => {
        ...
        const next = () => new Promise((res, reject) => {
            if (resolve(res, reject))
                return;
            resolvers.push(() => {
                resolve(res, reject);
            });
    ...
    _connect() {
        ...
        socket.on('message', (data, isBinary) => {
            ...
            if (event === null ||
                ...
                this._onError(null, 'received invalid WebSocket event: expected an object with an own string type', undefined);
                return;
            }
            const eventType = event.type;
            const reservedEventType = eventType === 'raw' ||
                eventType === 'close' ||
                eventType === 'event' ||
